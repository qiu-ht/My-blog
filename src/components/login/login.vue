            ...
            <el-tab-pane label="登录" name="second">
                <el-form :model="login" status-icon :rules="rules" ref="login"  label-width="100px" class="demo-ruleForm">
                    <el-form-item label="用户名" prop="loginUsername">
                        <el-input v-model="login.loginUsername" placeholder="请输入用户名"></el-input>
                    </el-form-item>
                    <el-form-item label="密码" prop="loginPassword">
                        <el-input placeholder="请输入密码" v-model="login.loginPassword" show-password></el-input>
                    </el-form-item>
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$api.user.register(user).then(
              res=>{
                alert(res.data);
              }
            )
